"""Regression test: parser output for ``KA-2627-6648.md`` must match
``KA-2627-6648.expected.json`` exactly.

Fixture is an Amazon Seller Services Tax Invoice with the same Details-of-Fees
layout as the credit note, but with positive amounts and a 6-column details
table (no Original Invoice columns).

Run with:
    python -m test.test_parser.test_kas_invoice
    python test/test_parser/test_kas_invoice.py
"""
...
def main():
    ...
    # ----- RCM must be False: doc explicitly says "Whether tax is payable
