"""Regression test: parser output for ``image (1).md`` must match
``image (1).expected.json`` exactly.

Fixture is an Anjanadri Enterprises tax invoice (image-sourced OCR)
with a 2-row ``<thead>`` header (rowspan + colspan sub-headers under
IGST), a 9-item body, and a "Total Quantity" footer row that must be
filtered out. Exercises:

- ``_flatten_header`` rowspan/colspan expansion for the IGST %/Amt
  sub-header.
- ``HSN/SAC`` column with bare HSN codes (no embedding).
- IGST-only tax layout (inter-state: seller Karnataka ``29`` vs buyer
  Telangana ``36``).
- Page-marker noise line ``H (8)`` (OCR artifact) filtered via
  :data:`_HEADER_NOISE_PATTERNS`.
- Bottom totals table with ``Total Amount Before Tax`` / ``Add : IGST``
  / ``Round Off Value`` / ``Amount With Tax`` label rows (vertical
  layout) — including the ``amount with tax`` alias for ``grand_total``.
- Bare phone / email lines (no ``Phone:`` label) in the seller block
  terminating address collection.

Run with:
    python -m test.test_parser.test_anjanadri
    python test/test_parser/test_anjanadri.py
"""
